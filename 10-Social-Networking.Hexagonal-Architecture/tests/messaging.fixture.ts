...
export const createMessagingFixture = () => {
  ...
  // };
  return {
    givenTheFollowingMessagesExist(messages: any[]) {
      // messageRepository.givenExistingMessages(messages);
    },
    givenNowIs(now: Date) {
      // dateProvider.now = now;
    },
    async whenUserPostsAmessage(postMessageCommand: any) {
      try {
    ...
    async whenUserEditsMessage(editMessageCommand: any) {
      try {
    ...
    async thenMessageShouldBe(expectedMessage: any) {
